...
class SafeMySQL_WP extends SafeMySQL {
	...
	public function insert( $table, $data ){
		return $this->query('INSERT INTO ?n SET ?u', $table, $this->__unset_invalid_fields( $data ) );
	}
	
	public function replace( $table, $data ){
		return $this->query('REPLACE INTO ?n SET ?u', $table, $this->__unset_invalid_fields( $data ) );
	}
	
	public function delete( $table, $where ){
		$where  = $this->__where_join( $where );
	...
	public function update( $table, $data, $where ){
		...
		$fields = $this->__unset_invalid_fields( $data );
		$where  = $this->__where_join( $where );
		
		return $this->query('UPDATE ?n SET ?u WHERE ?p', $table, $fields, $where );
	}
	
	## unset array element if value is empty: null or array(). For: inser, replace, update methods
	protected function __unset_invalid_fields( $data ){
		foreach( $data as $k => $val ){
			if( $val === null || $val === array() ) unset( $data[ $k ] );
		}
		return $data;
	}
	
	## NOTE: return sql save string
	protected function __where_join( $data ){
	...
	*/
	public function get_var(){
		return call_user_func_array( array( $this, 'getOne'), func_get_args() );
	}
	
	public function get_row(){
		$res = call_user_func_array( array( $this, 'getRow'), func_get_args() );
		if( is_array($res) )
			return (object) $res;
	...
	public function get_col(){
		$res = call_user_func_array( array( $this, 'getCol'), func_get_args() );
		if( is_array($res) )
			return (object) $res;
	...
	public function get_results(){
		$res = call_user_func_array( array( $this, 'getAll'), func_get_args() );
		if( is_array($res) ){
			foreach( $res as & $val ) $val = (object) $val;
		}
		return $res;
	}
}
