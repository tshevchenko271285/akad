...
function akad_widget() {
	register_widget( 'akad_instagram_widget' );
}
add_action( 'widgets_init', 'akad_widget' );

Class akad_instagram_widget extends WP_Widget {
			...
			if ( is_wp_error( $media_array ) ) {
				...
				foreach( $media_array as $item ) {
					...
					if ( locate_template( $template_part ) !== '' ) {
						...
						echo '<div class="' . esc_attr( $liclass ) . ' thumb" style="margin-bottom:15px"><a href="' . esc_url( $item['link'] ) . '" target="' . esc_attr( $target ) . '"  class="' . esc_attr( $aclass ) . '"><img src="' . esc_url( $item[$size] ) . '"  alt="' . esc_attr( $item['description'] ) . '" title="' . esc_attr( $item['description'] ) . '"  class="' . esc_attr( $imgclass ) . '"/></a></div>';
					}
				}
				?></ul><?php
			}
